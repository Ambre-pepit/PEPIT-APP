...
const SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRydGV2bHRod2x6ZWJqdnVzdHVmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MTE5NDY4MywiZXhwIjoyMDk2NzcwNjgzfQ.Fh7F42KqeNjBYQi1axKih6APkoifWTd-IF-6D29kBAU";
...
async function uploadLogo(file) {
  ...
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/Logos/${fileName}`, {
    method: "POST",
    headers: { "apikey": SERVICE_KEY, "Authorization": `Bearer ${SERVICE_KEY}`, "Content-Type": file.type },
    body: file,
  });
...
async function addPartenaire(data) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/partenaires`, { method: "POST", headers: { ...adminHeaders, "Prefer": "return=representation" }, body: JSON.stringify(data) });
  return res.json();
}

async function updatePartenaire(id, data) {
  await fetch(`${SUPABASE_URL}/rest/v1/partenaires?id=eq.${id}`, { method: "PATCH", headers: adminHeaders, body: JSON.stringify(data) });
}

async function deletePartenaire(id) {
  await fetch(`${SUPABASE_URL}/rest/v1/partenaires?id=eq.${id}`, { method: "DELETE", headers: adminHeaders });
}
