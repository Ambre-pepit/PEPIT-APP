...
export default function App() {
  ...
  // Partenaires
  function openAddPart() { setEditPart(null); setPartNom(""); setPartCat("Mode"); setPartDept("Guadeloupe"); setPartRemise(""); setPartAdresse(""); setPartLogo(""); setShowPartForm(true); }
  function openEditPart(p) { setEditPart(p); setPartNom(p.nom); setPartCat(p.categorie); setPartDept(p.departement); setPartRemise(p.remise); setPartAdresse(p.adresse || ""); setPartLogo(p.logo_url || ""); setShowPartForm(true); }
  async function savePart() {
    const data = { nom: partNom, categorie: partCat, departement: partDept, remise: partRemise, adresse: partAdresse, logo_url: partLogo, actif: true };
        ...
        <div style={styles.content}>
          ...
          {showPartForm && (
            <div style={{ background: "white", border: `1px solid ${GOLD}`, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <input style={styles.input} placeholder="URL du logo (ex: https://...)" value={partLogo} onChange={e => setPartLogo(e.target.value)} />
