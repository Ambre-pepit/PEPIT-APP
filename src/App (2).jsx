...
async function signUp(email, password, nom, offre) {
  ...
  if (userId) {
    await fetch(`${SUPABASE_URL}/rest/v1/membres`, {
      ...
      body: JSON.stringify({ nom, email, offre, statut: "actif", economies: 0, nb_scans: 0, auth_id: userId })
    });
  }
  return data;
}
...
async function getMembre(token, userId) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/membres?auth_id=eq.${userId}&select=*`, {
    headers: { ...dbHeaders, "Authorization": `Bearer ${token}` }
  });
  const data = await res.json();
  return data[0] || null;
}

async function getPartenaires(token) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/partenaires?actif=eq.true&order=nom`, {
    headers: { ...dbHeaders, "Authorization": `Bearer ${token}` }
  });
  return res.json();
}
...
async function verifyMembre(code) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/membres?id=eq.${code}&select=*`, {
    headers: dbHeaders
  });
  const data = await res.json();
  return data[0] || null;
...
export default function App() {
  ...
  useEffect(() => {
    if (screen === "main" && token) loadPartenaires();
  }, [screen, token]);

  async function loadPartenaires() {
    try {
      const data = await getPartenaires(token);
      setPartenaires(data);
  ...
  async function doVerify() {
    const membre = await verifyMembre(manualCode.trim());
    if (membre) setScanResult(membre);
  ...
  if (screen === "reset") return (
    <div style={styles.app}>
      ...
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ textAlign: "center", fontSize: 12, color: "#888" }}>
          <span onClick={() => { setScreen("login"); setError(""); }} style={{ color: GOLD, cursor: "pointer", textDecoration: "underline" }}>Retour à la connexion</span>
        </div>
      </div>
    </div>
  );
