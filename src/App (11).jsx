...
export default function App() {
  ...
  // Partenaires
  function openAddPart() { setEditPart(null); setPartNom(""); setPartCat("Mode"); setPartDept("Guadeloupe"); setPartRemise(""); setPartAdresse(""); setPartLogo(""); setPartLogoFile(null); setShowPartForm(true); }
  function openEditPart(p) { setEditPart(p); setPartNom(p.nom); setPartCat(p.categorie); setPartDept(p.departement); setPartRemise(p.remise); setPartAdresse(p.adresse || ""); setPartLogo(p.logo_url || ""); setPartLogoFile(null); setShowPartForm(true); }
  async function savePart() {
    let logoUrl = partLogo;
    if (partLogoFile) {
      try { logoUrl = await uploadLogo(partLogoFile); } catch(e) { alert("Erreur upload logo"); return; }
    }
    const data = { nom: partNom, categorie: partCat, departement: partDept, remise: partRemise, adresse: partAdresse, logo_url: logoUrl, actif: true };
