    ...
    <div style={styles.app}>
      ...
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <button style={styles.btnPrimary} onClick={async () => {
          if (!entrepriseNom || !entrepriseEmail) { setError("Veuillez remplir le nom et l'email"); return; }
          if (entrepriseNbSalaries < 7) { setError("L'offre entreprise est disponible à partir de 7 salariés"); return; }
          setLoading(true); setError("");
          try {
            await addEntreprise({ nom: entrepriseNom, email: entrepriseEmail, telephone: entrepriseTel, nb_salaries: entrepriseNbSalaries, offre_base: entrepriseOffre, statut: "en_attente", valide: false });
            const url = createEntrepriseCheckout(entrepriseOffre, entrepriseEmail, entrepriseNbSalaries);
            setSuccess("✅ Inscription enregistrée ! Vous allez être redirigé vers le paiement...");
  ...
  if (screen === "espace-entreprise") return (
    <div style={styles.app}>
      ...
      {adminTab === "salaries" && (
        <div style={styles.content}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: 14, fontWeight: 600 }}>{salariesEntreprise.length} salarié(s)</div>
          </div>
          {salariesEntreprise.length === 0 && (
            <div style={{ textAlign: "center", color: "#aaa", fontSize: 13, padding: 20 }}>
              Aucun salarié inscrit pour l'instant.<br/>
              <span style={{ fontSize: 12 }}>Contactez admin.pepit@gmail.com pour inviter vos salariés.</span>
            </div>
          )}
          {salariesEntreprise.map(s => (
      ...
      {tab === "account" && showParametres && (
        <div>
          ...
          <div style={styles.content}>
            ...
            <div style={{ fontSize: 11, color: "#aaa", textAlign: "center" }}>PEPIT' SAS — Version 1.0<br />Contact : contact@pepit.gp</div>
          </div>
        </div>
      )}
