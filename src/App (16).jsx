...
const STRIPE_KEY = "pk_test_51TgmfpV05dZ94v14slviD5ySBzXCKQTGv9NYGLd1JLVfwm19qQp2uFKbhUEtapo9SwEFUVGSKlFbufsYAjCTR09g00cqG4h7V4";
const STRIPE_PRICES = {
...
async function createCheckoutSession(offre, email) {
  const res = await fetch("https://api.stripe.com/v1/checkout/sessions", {
    ...
    body: new URLSearchParams({
      ...
      "mode": "subscription",
      "line_items[0][price]": STRIPE_PRICES[offre],
      "line_items[0][quantity]": "1",
      "customer_email": email,
      "success_url": "https://pepit-app-indol.vercel.app/?payment=success",
      "cancel_url": "https://pepit-app-indol.vercel.app/?payment=cancel"
    })
  });
...
export default function App() {
  ...
  async function doSignup() {
    ...
    try {
      ...
      const checkoutUrl = await createCheckoutSession(signupOffre, signupEmail.trim().toLowerCase());
      if (checkoutUrl) {
        ...
        <button style={styles.btnPrimary} onClick={async () => {
          ...
          try {
            ...
            await fetch(`${SUPABASE_URL}/rest/v1/partenaires`, {
              ...
              body: JSON.stringify({ nom: signupNom, email: signupEmail.trim().toLowerCase(), categorie: signupOffre, auth_id: data.user?.id, valide: false, actif: false })
            });
              ...
              {[["Essentiel", "9,90€/mois", "1 département"], ["Trio", "14,90€/mois", "3 départements"], ["Prestige", "24,90€/mois", "3 dép. + goodies + événements"]].map(([offre, prix, desc]) => (
                <div key={offre} onClick={async () => {
                  ...
                  if (window.confirm(`Changer votre offre pour ${offre} à ${prix} ?`)) {
                    const url = await createCheckoutSession(offre, user.email);
