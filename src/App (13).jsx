  ...
  async function doLogin() {
    ...
    try {
      ...
      if (loginType === "merchant") {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/partenaires?auth_id=eq.${data.user.id}&select=*`, { headers: { ...dbHeaders, "Authorization": `Bearer ${data.access_token}` } });
        ...
        <button style={styles.btnPrimary} onClick={async () => {
          ...
          try {
            ...
            await fetch(`${SUPABASE_URL}/rest/v1/partenaires`, {
              ...
              body: JSON.stringify({ nom: signupNom, email: signupEmail.trim().toLowerCase(), categorie: signupOffre, auth_id: data.user?.id, valide: false, actif: false, nb_scans: 0 })
            });
