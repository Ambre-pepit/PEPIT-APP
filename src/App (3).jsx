...
const ADMIN_EMAIL = "ambre.placide@gmail.com";
...
async function signUp(email, password, nom, offre) {
  ...
  if (data.user?.id) {
    await fetch(`${SUPABASE_URL}/rest/v1/membres`, { method: "POST", headers: { ...dbHeaders, "Authorization": `Bearer ${data.access_token}` }, body: JSON.stringify({ nom, email, offre, statut: "actif", economies: 0, nb_scans: 0, auth_id: data.user.id }) });
  }
  return data;
}
...
export default function App() {
  ...
  useEffect(() => {
    if (screen === "main" || screen === "admin") loadPartenaires();
    if (screen === "admin") loadMembres();
  }, [screen]);

  async function loadPartenaires() {
    try { setPartenaires(await getPartenaires()); } catch (e) { }
  }

  async function loadMembres() {
    try { setMembres(await getMembres()); } catch (e) { }
  }
  ...
  function doLogout() {
    ...
    setScreen("login"); setScanResult(null); setEmail(""); setPass(""); setSuccess("");
  }
  ...
  function openAddPart() {
    setEditPart(null); setPartNom(""); setPartCat("Mode"); setPartDept("Guadeloupe"); setPartRemise("");
    setShowPartForm(true);
  }

  function openEditPart(p) {
    setEditPart(p); setPartNom(p.nom); setPartCat(p.categorie); setPartDept(p.departement); setPartRemise(p.remise);
    setShowPartForm(true);
  }

  async function savePart() {
    const data = { nom: partNom, categorie: partCat, departement: partDept, remise: partRemise, actif: true };
    if (editPart) { await updatePartenaire(editPart.id, data); }
    else { await addPartenaire(data); }
  ...
  if (screen === "reset") return (
    <div style={styles.app}>
      <div style={{ background: DARK2, padding: "32px 24px", textAlign: "center", borderBottom: `1px solid ${GOLD}44` }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(186,117,23,0.6)", marginTop: 4 }}>Mot de passe oublié</div>
      </div>
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ textAlign: "center", fontSize: 12, color: "#888" }}>
          <span onClick={() => { setScreen("login"); setError(""); }} style={{ color: GOLD, cursor: "pointer", textDecoration: "underline" }}>Retour</span>
        </div>
      </div>
    </div>
  );
  ...
  if (screen === "admin") return (
    <div style={{ ...styles.app, maxWidth: 600 }}>
        ...
        <div style={styles.content}>
          ...
          {showPartForm && (
            <div style={{ background: "white", border: `1px solid ${GOLD}`, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <select style={styles.input} value={partDept} onChange={e => setPartDept(e.target.value)}>
      ...
      {adminTab === "membres" && (
        <div style={styles.content}>
          <div style={{ fontSize: 14, fontWeight: 600 }}>{membres.length} membre(s) inscrit(s)</div>
          {membres.map(m => (
