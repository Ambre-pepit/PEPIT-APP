...
const styles = {
  ...
  header: { background: DARK, color: "white", padding: "16px", textAlign: "center", position: "relative" },
  ...
  if (screen === "login") return (
    <div style={styles.app}>
      ...
      <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ textAlign: "center", fontSize: 12, color: "#888" }}>Pas encore membre ? <span style={{ color: GOLD, cursor: "pointer", textDecoration: "underline" }}>S'inscrire</span></div>
      </div>
    </div>
  );
        ...
        {scanResult && !scanResult.error && (
          <div style={{ background: "#d4edda", borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ marginTop: 10, background: "white", borderRadius: 8, padding: 10, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: DARK, marginTop: 2 }}>{scanResult.discount}</div>
            </div>
          </div>
        )}
          ...
          <div style={styles.memberCard}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <div style={{ marginTop: 8 }}><span style={{ ...styles.badge, background: "#2a1a00", color: GOLD, border: `1px solid ${GOLD}44` }}>{user?.offer}</span></div>
              </div>
              <div style={{ fontSize: 22, fontWeight: 800, color: GOLD, letterSpacing: 3 }}>PEPIT'</div>
            </div>
            <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ background: GOLD, borderRadius: 8, padding: "8px 12px", fontSize: 11, color: DARK, fontWeight: 800, letterSpacing: 1 }}>QR</div>
              <div>
                <div style={{ fontSize: 10, color: "#aaa" }}>N° membre</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: GOLD }}>{user?.id}</div>
                <div style={{ fontSize: 10, color: "#aaa", marginTop: 4 }}>Économies ce mois</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: "white" }}>{user?.savings}</div>
              </div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div style={{ background: DARK, borderRadius: 8, padding: 12, textAlign: "center", border: `1px solid ${GOLD}33` }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: GOLD }}>{user?.scans}</div>
              <div style={{ fontSize: 11, color: "#aaa" }}>utilisations</div>
            </div>
            <div style={{ background: DARK, borderRadius: 8, padding: 12, textAlign: "center", border: `1px solid ${GOLD}33` }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: GOLD }}>{user?.savings}</div>
          ...
          {filteredPartners.map(p => (
            <div key={p.id} style={{ ...styles.card, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div style={{ marginTop: 6 }}><span style={{ ...styles.badge, background: "#1a1a00", color: GOLD }}>{p.discount}</span></div>
              </div>
      ...
      {tab === "account" && <>
        ...
        <div style={styles.content}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 48, height: 48, borderRadius: "50%", background: DARK, border: `2px solid ${GOLD}`, display: "flex", alignItems: "center", justifyContent: "center", color: GOLD, fontWeight: 700, fontSize: 16 }}>{user?.name?.split(" ").map(n => n[0]).join("")}</div>
            <div>
          ...
          <div style={styles.card}>
            ...
            <span style={{ ...styles.badge, background: "#1a1a00", color: GOLD }}>{user?.offer} · {user?.offer === "Prestige" ? "24,90€" : user?.offer === "Trio" ? "14,90€" : "9,90€"}/mois</span>
